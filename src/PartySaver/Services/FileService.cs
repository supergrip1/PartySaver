...
public class FileService
{
    ...
    public async Task<Guid> AddFile(Stream data, string name)
    {
        ...
        var relativePath = Path.Combine(folder1, folder2, guid);
        var fullPath = Path.Combine(DirectoryHelper.GetImageDirectory(), relativePath);

        Directory.CreateDirectory(fullPath);

        var relativeFile = Path.Combine(relativePath, "image");
        var fullFile = Path.Combine(fullPath, "image");

        var fs = File.Create(fullFile);
